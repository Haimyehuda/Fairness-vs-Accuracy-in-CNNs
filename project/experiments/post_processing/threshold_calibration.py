# ===============================================================
# POST-PROCESSING STEP 2 — THRESHOLD CALIBRATION
# File:
#   project/experiments/post_processing/threshold_calibration.py
#
# Purpose:
#   - Implement Threshold Calibration for Equal Opportunity as a
...
#
# Run with:
#   python -m project.experiments.post_processing.threshold_calibration \
#     --method "Baseline" \
#     --scenario 10-90
# ===============================================================
...
def safe_name(text: str) -> str:
    ...
    """
    # Use mapping first if present, otherwise fall back to string replacements
    cleaned = text.strip()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run post-processing threshold calibration to satisfy Equal Opportunity."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory where predictions are stored. If None, derived from drive-root.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory where sweeps are saved. If None, derived from drive-root.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional directory where best result is saved. If None, derived from drive-root.",
    )
...
def main():
    ...
    predictions_dir = args.predictions_dir if args.predictions_dir else os.path.join(drive_root, "predictions")
    sweeps_dir = args.sweeps_dir if args.sweeps_dir else os.path.join(drive_root, "post_processing", "threshold_calibration", "sweeps")
    results_dir = args.results_dir if args.results_dir else os.path.join(drive_root, "post_processing", "threshold_calibration", "results")
    ...
    print("=============================================================")
    print(" POST-PROCESSING STEP 2 — THRESHOLD CALIBRATION")
    print("=============================================================")
    ...
    required_cols = ["y_true_id", "prob_PNEUMONIA"]
    ...
    sweep_results = []
    for t in thresholds:
        # Prediction rule: if prob_PNEUMONIA >= threshold => PNEUMONIA (1) else NORMAL (0)
        y_pred = (prob_pneumonia >= t).astype(int)
    ...
    # Priority 1: Minimize ΔTPR (Equal Opportunity)
    # Priority 2: Maximize Accuracy (Overall)
    # Priority 3: Closest to 0.50
    sweep_df_temp = sweep_df.copy()
    sweep_df_temp["dist_to_0_5"] = (sweep_df_temp["threshold"] - 0.50).abs().round(6)

    sorted_df = sweep_df_temp.sort_values(
    ...
    # Extract original threshold 0.50 metrics for comparison
    row_050 = sweep_df[np.isclose(sweep_df["threshold"], 0.50)].iloc[0]
    ...
    # Save sweep results
    sweep_path = os.path.join(sweeps_dir, f"{method_safe}_{scenario_safe}_threshold_sweep.csv")
    sweep_df_save = sweep_df.rename(columns={"threshold": "Threshold"})[SWEEP_COLUMNS]
    sweep_df_save.to_csv(sweep_path, index=False)

    # Save selected best result
    result_path = os.path.join(results_dir, f"{method_safe}_{scenario_safe}_threshold_calibrated_result.csv")
    best_result = {
    ...
    print("\n=============================================================")
    print(" THRESHOLD CALIBRATION SUMMARY COMPARISON")
    print("=============================================================")
    print(f"Method:            {args.method}")
    print(f"Scenario:          {args.scenario}")
    ...
    print("-------------------------------------------------------------")
    print(f"{'Metric':<30} | {'Threshold 0.50 (Original)':<25} | {f'Threshold {selected_threshold:.2f} (Calibrated)':<25}")
    print("-" * 86)

    print(f"{'Accuracy (Overall)':<30} | {row_050['Accuracy (Overall)']:<25.4f} | {best_row['Accuracy (Overall)']:<25.4f}")
    print(f"{'Accuracy NORMAL':<30} | {row_050['Accuracy NORMAL']:<25.4f} | {best_row['Accuracy NORMAL']:<25.4f}")
    print(f"{'Accuracy PNEUMONIA':<30} | {row_050['Accuracy PNEUMONIA']:<25.4f} | {best_row['Accuracy PNEUMONIA']:<25.4f}")
    print(f"{'Recall / TPR NORMAL':<30} | {row_050['Recall / TPR NORMAL']:<25.4f} | {best_row['Recall / TPR NORMAL']:<25.4f}")
    print(f"{'Recall / TPR PNEUMONIA':<30} | {row_050['Recall / TPR PNEUMONIA']:<25.4f} | {best_row['Recall / TPR PNEUMONIA']:<25.4f}")
    print(f"{'F1 NORMAL':<30} | {row_050['F1 NORMAL']:<25.4f} | {best_row['F1 NORMAL']:<25.4f}")
    print(f"{'F1 PNEUMONIA':<30} | {row_050['F1 PNEUMONIA']:<25.4f} | {best_row['F1 PNEUMONIA']:<25.4f}")
    print(f"{'ΔTPR (Equal Opportunity)':<30} | {row_050['ΔTPR (Equal Opportunity)']:<25.4f} | {best_row['ΔTPR (Equal Opportunity)']:<25.4f}")
    print(f"{'ΔFPR (Equalized Odds)':<30} | {row_050['ΔFPR (Equalized Odds)']:<25.4f} | {best_row['ΔFPR (Equalized Odds)']:<25.4f}")
    print(f"{'Disparate Impact (DI)':<30} | {row_050['Disparate Impact (DI)']:<25.4f} | {best_row['Disparate Impact (DI)']:<25.4f}")

    print("-------------------------------------------------------------")
    print("Output Files Saved:")
    print(f"  Sweep results:      {sweep_path}")
    print(f"  Calibrated result:  {result_path}")
    print("=============================================================")
